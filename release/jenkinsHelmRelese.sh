...
case $REPOSITORY in
  demo42/helloworld*)
    echo helm upgrade helloworld ./release/helm/ --reuse-values --set helloworld.image=$REGISTRY/$REPOSITORY:$TAG 
    helm upgrade helloworld ./release/helm/ --reuse-values --set helloworld.image=$REGISTRY/$REPOSITORY:$TAG 
